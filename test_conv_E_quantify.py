...
def generate_conv_E_quantify_tests():
  ...
  quantify_types = [0,1,2] #(E=1)&&(scale=0), scale=0, E和scale任一uint8数
...
def standard_conv_E_quantify(conv_type, mode_type, quantify_type):
  ...
  E_data, E_ddr_words = generate_conv_E_data(quantify_type, mode, of)
  scale_data, scale_ddr_words = generate_conv_scale_data(quantify_type, mode, of, k)
...
#fpga conv
def fpga_conv_E_quantify(conv_type, mode_type, quantify_type):
  # def basic conv op
  mode = mode_type
  k,s,p = conv_type_mapping[conv_type]
  of = 128
